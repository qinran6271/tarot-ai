...
type UseReadingChatParams = {
  ...
  setConversation: Dispatch<SetStateAction<ReadingMessage[]>>;
};
...
export function useReadingChat({
  ...
  async function sendMessage(message?: string) {
    ...
    try {
      ...
      // 解析响应
      const data = (await response.json()) as ReadingChatResponse;
      ...
      const finalConversation = [...conversationWithUser, assistantMessage];
      ...
      console.error("Reading chat request failed:", error);

      // 请求失败时撤销临时显示的用户消息，
      // 避免失败消息进入正式 Conversation。
      setConversation(conversation);

      // 恢复输入内容，方便用户重新发送。
      setInput(text);

      // 错误是临时 UI 状态，不保存进 Reading。
      setChatError(
